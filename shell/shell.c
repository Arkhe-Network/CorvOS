...
void shell_run() {
    ...
    while (1) {
        ...
        else if (strcmp(buffer, "avatar") == 0) {
            console_writeln("Avatar: Initiating corporeal manifestation...");
            system("python3 user/demo_avatar.py");
        }
        else if (strcmp(buffer, "safe-start") == 0) {
            console_writeln("Avatar: Executing Safe Start Protocol...");
            system("python3 user/safe_start.py");
        }
        ...
        else if (strcmp(buffer, "help") == 0) console_writeln("Commands: ls, ps, mem, net, arkhe, urban-sync, meditate, chain-print, spawn-agent, bridge-formalize, fold, cad, fem, music, calc, avatar, safe-start, run, vm, help, exit");
